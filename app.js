...
const appState = { supabase: null, user: null, currentNovel: null, currentChapters: [] };
...
function getUserLabel(user) { return user?.email || 'Autor'; }
function getUserInitial(user) { return (user?.email || 'A').trim().charAt(0).toUpperCase(); }

function showHomeMessage() {
  if (!document.body || document.getElementById('account-message')) return;
  const params = new URLSearchParams(window.location.search);
  const message = params.get('confirmed') === '1'
    ? 'Correo confirmado correctamente. Tu cuenta ya está activa.'
    : params.get('welcome') === '1'
      ? 'Sesión iniciada correctamente. Bienvenido/a a Editorial Acuario.'
      : '';
  if (!message) return;
  const banner = document.createElement('div');
  banner.id = 'account-message';
...
async function loadPublicNovels() {
  ...
  if (error) { console.error('No se pudieron cargar las novelas:', error); return; }
